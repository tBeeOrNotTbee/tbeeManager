function bigUpload () {
	...
	this.settings = {
		...
		'scriptPath': "/manager/content/bigUpload",
	...
	this.success = function(response) {
		$("#computed_hash").val(response.computed_hash);	
	...
	this.processFiles = function() {
		...
		this.$(this.settings.responseField).textContent = 'Subiendo...';
		this.$(this.settings.submitButton).value = 'Pausar';
		arr_file_input = this.$(this.settings.inputField).files[0];					
        //reader.readAsBinaryString(arr_file_input);				
		version = $("#bigUploadFile").attr("version");
		file_name = arr_file_input['name'].substring(0, arr_file_input['name'].lastIndexOf('.'));
		extension = arr_file_input['name'].slice((Math.max(0, arr_file_input['name'].lastIndexOf(".")) || Infinity) + 1);
		if ((file_name.indexOf("+")>0)||(file_name.indexOf("&")>0)) {
			this.$(this.settings.submitButton).value = 'Subir';
			this.printResponse('El archivo no puede poseer el caracter + o & en el nombre.', true);
			return;
		}
		if (version>0 && file_name != "background") {	
			var fileVersion = parseInt(version)+1;		
			tbeeContentName = file_name + "_" + fileVersion + ".";						
		} else
			tbeeContentName = file_name  + ".";	
		tbeeFile = tbeeContentName + extension;
		this.uploadData.tbeeFileName = tbeeFile;
		//Alias the file input object to this.uploadData
		this.uploadData.file = arr_file_input;			
		this.uploadData.customerId = $("#bigUploadFile").attr("customerId");
		...
		if (fileSize > this.settings.maxFileSize) {
			this.$(this.settings.submitButton).value = 'Subir';
			this.printResponse('El archivo supera el tamaño máximo permitido.', true);
			return;
		}
	...
	this.sendFile = function (chunk) {
		...
		if (this.uploadData.paused === true) {
			...
			this.printResponse('Subida pausada.', false);
			return;
		}
		...
		reader.onload = function(evt) {
			//contenido en el buffer
			var chars  = new Uint8Array(evt.target.result);
			  var CHUNK_SIZE = parent.settings.chunkSize; 
			  var index = 0;
			...
			xhr.open("POST", parent.settings.scriptPath + '/upload/'+hashBlob+'/' + parent.uploadData.key + parent.settings.scriptPathParams, true);
			xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			//xhr.setRequestHeader("Content-type", "application/json");
			//console.log( parent.settings.scriptPath + '/upload/'+hashBlob+'/' + parent.uploadData.key + parent.settings.scriptPathParams);
			if (Crypto.MD5(result)!='d41d8cd98f00b204e9800998ecf8427e')
				parent.settings.spark.append(evt.target.result); 
	...
	this.sendFileData = function() {
		computed_hash = parent.settings.spark.end();
		var data = 'key=' + this.uploadData.key + '&name=' + this.uploadData.tbeeFileName + '&customerId=' + this.uploadData.customerId;
		xhr = new XMLHttpRequest();
		xhr.open("POST", parent.settings.scriptPath + '/finish/' + computed_hash, true);
		xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xhr.onreadystatechange = function() {
				if (xhr.readyState == 4) {
					var response = JSON.parse(xhr.response);
					...
					parent.$(parent.settings.submitButton).value = 'Subir';
					parent.printResponse('Archivo cargado exitosamente.', false);
	...
	this.abortFileUpload = function() {
		...
		xhr.onreadystatechange = function() {
				if (xhr.readyState == 4) {
					...
					parent.printResponse('La subida del archivo fue cancelada.', true);
				}
			};
		//Send the request
		xhr.send(data);
	...
	this.progressUpdate = function(progress) {
		...
		if (progress % 5 === 0) {
			...
			this.$(this.settings.timeRemainingField).textContent = timeLeft + ' segundos faltantes';
		}
	};
