...
<div id="content-main" class="round dropshadow" >
	...
	<h4>Ingresar contenido</h4>
    <form id="formContent" role="form" action="<?php echo FRONT_CONTROLLER ?>content/addFile" method="post">
		<fieldset>
			<select id="selectContentType" class="form-control"  name="selectContentType">
				<option value=0>Tipo de contenido...</option>
				<option value=<?php echo TYPE_VIDEO ?>>Video</option>
				<option value=<?php echo TYPE_IMAGE ?>>Imagen</option>
				<option value=<?php echo TYPE_WEB ?>>Web</option>
				<option value=<?php echo TYPE_MENU ?>>Menú</option>
				<option value=<?php echo TYPE_LIST ?>>Lista</option>
				<option value=<?php echo TYPE_ICON ?>>Icon</option>
				<option value=<?php echo TYPE_BACKGROUND ?>>Background</option>
				<option value=<?php echo TYPE_YOUTUBE ?>>YouTube</option>
				<option value=<?php echo TYPE_AUDIO ?>>Audio</option>
				<option value=<?php echo TYPE_LABEL ?>>Label</option>
				<option value=<?php echo TYPE_WATERMARK ?>>Watermark</option>
			</select>	
				...
				<div class="bigUpload">
					...
					<p class="help-block">No debe superar los 300 MB.</p>
						<form method="post" enctype="multipart/form-data" id="bigUploadForm">
							<input type="file" id="bigUploadFile" class="form-control" name="bigUploadFile" customerId="<?php echo $data['idCustomer'] ?>" />
			...
			<div id="youtube" class="top-buffer">		
				...
				<input class="form-control" type="checkbox" name="youtubePlaylist" id="youtubePlaylist" value="true" />		
			</div>
			...
			<button id="submit" class="form-control btn btn-info top-buffer" type="submit" name="submit">Enviar</button>
...
$(document).ready(function(){
	...
	$("#selectContentType").change(function(){
		formType = $("#selectContentType").val();
		hideFormElements();
